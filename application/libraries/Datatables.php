...
//ERR
	...
	public function group($cond){
		//$groupData = $this->db->group_by($cond);
		$this->groupData['grupby'] = $cond;
		return $this;
	}

	public function order($title,$short){
		//$this->db->order_by($title, $short);
		$this->orderby['urut'][] = $title; 
		$this->orderby['urut'][] = $short;
		return $this;
	}

	public function draw(){
		if(!empty($this->groupData['grupby'])){
			$this->db->group_by($this->groupData['grupby']);
		}
		if(!empty($this->orderby['urut'])){
			$ARorder = $this->orderby['urut'];
			$this->db->order_by($ARorder[0], $ARorder[1]);
		}
	...
	private function get_paging($keyword){
		$hit = "";
		if(!empty($this->groupData['grupby'])){
			$groupby = $this->groupData['grupby'];
			$this->do_join();
			$this->db->group_by($groupby);
			$hit = $this->db->get($this->table)->num_rows();
		}
